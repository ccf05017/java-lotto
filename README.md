...
## 2차 피드백 반영
- [ ] NPE 방지를 위해 validation에서 null 체크를 가장 처음에 수행하도록 개선
- [ ] 코드 컨벤션에서 정적 팩터리 메서드도 생성에 관여하기 때문에 생성자급으로 취급한다. 단, 생성자보다는 하부에 위치
- [ ] 연산자 오버로딩 가능한 배제하도록 리팩토링
- [ ] 메서드명을 실제 수행하는 동작에 어울리게 수정
